...
export default function Post({ post, addReply, handleVote, depth }: PostProps) {
  ...
  const handleVoteClick = (isLike: boolean) => {
    if (!hasVoted.like && !hasVoted.dislike) {
  ...
  return (
    <motion.div
      ...
      id={post.id}
      className={`bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md dark:shadow-gray-700 ${marginClass} mb-4`}
    >
      <div className="flex flex-col">
        <div className="flex justify-between items-start mb-2">
          <p className="text-gray-800 dark:text-gray-100">{post.content}</p>
          <span className="text-sm text-gray-500 dark:text-gray-400 mr-4">{formatTimestamp(post.timestamp)}</span>
        </div>
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className={`flex items-center gap-1 px-2 ${
                hasVoted.like ? "text-green-500 dark:text-green-400" : "text-gray-600 dark:text-gray-300"
              }`}
              disabled={hasVoted.like || hasVoted.dislike}
            >
            ...
            <Button
              ...
              className={`flex items-center gap-1 px-2 ${
                hasVoted.dislike ? "text-red-500 dark:text-red-400" : "text-gray-600 dark:text-gray-300"
              }`}
              disabled={hasVoted.like || hasVoted.dislike}
            >
      ...
      <div className="mt-4">
        {post.replies.map((reply) => (
          <Post key={reply.id} post={reply} addReply={addReply} handleVote={handleVote} depth={depth + 1} />
        ))}
      </div>
    </motion.div>
  )
